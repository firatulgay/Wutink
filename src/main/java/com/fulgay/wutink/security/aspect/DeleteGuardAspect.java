package com.fulgay.wutink.security.aspect;

import com.fulgay.wutink.domain.Experience;
import com.fulgay.wutink.security.annotation.DeleteGuard;
import com.fulgay.wutink.security.annotation.IdGuard;
import com.fulgay.wutink.security.jwt.manager.EncryptionManager;
import com.fulgay.wutink.security.jwt.manager.JwtTokenManager;
import com.fulgay.wutink.service.ExperienceService;
import com.fulgay.wutink.service.SessionService;
import com.fulgay.wutink.service.UserService;
import org.aspectj.lang.JoinPoint;
import org.aspectj.lang.annotation.Aspect;
import org.aspectj.lang.annotation.Before;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;
import org.springframework.util.StringUtils;
import org.springframework.web.context.request.RequestContextHolder;
import org.springframework.web.context.request.ServletRequestAttributes;

import javax.servlet.http.HttpServletRequest;
@Aspect
@Component
public class DeleteGuardAspect {
    @Autowired
    private JwtTokenManager tokenManager;

    @Autowired
    private EncryptionManager encryptionManager;

    @Autowired
    private ExperienceService experienceService;

    @Autowired
    private SessionService sessionService;



    @Before("@annotation(deleteGuard)")
    public void execute(JoinPoint joinPoint, DeleteGuard deleteGuard) {

        int argIndex = deleteGuard.parameterIndex();
        if(argIndex < 0){
            argIndex = 0;
        }

        Object[] args = joinPoint.getArgs();
        if(args == null || args.length == 0) {
            throw new RuntimeException("ACCESS ERROR FOR INVALID RESOURCE");
        }

        Object experienceId = args[argIndex];
        if(experienceId == null) {
            throw new RuntimeException("ACCESS ERROR FOR INVALID RESOURCE");
        }

        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.currentRequestAttributes()).getRequest();
        String token = tokenManager.extractJwtFromRequest(request);
        if(!StringUtils.hasText(token)) {
            throw new RuntimeException("ACCESS ERROR FOR INVALID RESOURCE");
        }

        if (!isUserExperienceOwner((Long) experienceId,sessionService.getSessionUserName())) {
            throw new RuntimeException("ACCESS ERROR FOR INVALID RESOURCE");
        }
    }

    private boolean isUserExperienceOwner(Long experienceId,String username){
        Experience experience = experienceService.findById(experienceId);
        return experience.getUser().getUserName().equals(username);
    }
}
